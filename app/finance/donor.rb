class Donor
    ...
    def self.merge_donations(receipts, in_kinds)
        clean_receipts = receipts.map do |receipt|
            { id: receipt.id, name: receipt.name, email: receipt.email, phone_number: receipt.phone_number, amount: receipt.amount, person: receipt.person, occupation: receipt.occupation, employer: receipt.employer, address: receipt.address, city: receipt.city, state: receipt.state, country: receipt.country }
        end

        clean_in_kinds = in_kinds.map do |in_kind|
            { id: in_kind.id, name: in_kind.from_whom, email: in_kind.email, phone_number: in_kind.phone_number, amount: in_kind.value, person: in_kind.person, address: in_kind.address, city: in_kind.city, state: in_kind.state, country: in_kind.country }
        end
